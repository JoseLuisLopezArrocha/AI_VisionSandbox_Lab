...
PERSISTENCE_PRESETS = [
    ("Instantaneo", 0), ("2s", 2), ("5s", 5), ("10s", 10), ("30s", 30)
]
COOLDOWN_PRESETS = [
    ("5s", 5), ("10s", 10), ("30s", 30), 
    ("1 min", 60), ("5 min", 300), ("Sin limite", 0),
]
VALIDATOR_PROVIDERS = [
    ("Sin validacion", "None"),
    ("YOLO-World (Universal)", "universal"),
    ("Segmentacion local", "local_seg"),
    ("Ollama (VLM)", "ollama"),
...
class EventsHelpWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        self._add_topic(scroll, "Nombre del Hito", 
                        "Es el identificador de la alerta. Aparecera en los logs y en los mensajes de Telegram/Webhook.")
        
        self._add_topic(scroll, "Clase", 
                        "El tipo de objeto que activara la regla. 'Cualquiera' sirve para detectar cualquier movimiento.")
        
        self._add_topic(scroll, "Condicion y Valor", 
                        "La logica matematica (>, <, ==). 'Total >' permite disparar alertas basadas en el conteo historico acumulado.")
        
        self._add_topic(scroll, "Zona", 
                        "Define si la regla aplica a todo el video (Global) o a una de las zonas poligonales que hayas dibujado.")
        
        self._add_topic(scroll, "Accion", 
                        "Puedes combinar multiples acciones: Log Visual, enviar a Telegram, Webhook o reproducir una alerta por Voz (TTS). Tambien puedes guardar una Foto de evidencia.")
        
        self._add_topic(scroll, "Persistencia y Severidad",
                        "La Persistencia evita falsas alarmas exigiendo que la deteccion se mantenga X segundos. La Severidad clasifica el hito como Info, Alerta o Critico.")
        ...
        self._add_topic(scroll, "Validacion Secundaria", 
                        "Permite usar un segundo modelo de IA para confirmar la deteccion antes de disparar la alerta.\n\n"
        ...
        self._add_topic(scroll, "Donde se guarda todo?", 
...
class EventsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, event_engine, available_classes, zones_count):
        ...
        # Titulo con boton de ayuda a la derecha
    ...
    def _build_creator_frame(self):
        ...
        self.entry_name = ctk.CTkEntry(frame, placeholder_text="Ej: Alerta multitud, Zona vacia...")
        self.entry_name.pack(pady=(0, 8), padx=12, fill="x")

        # Fila 2: Condicion (Clase + Operador + Valor)
        ...
        ctk.CTkLabel(op_col, text="Condicion:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        ...
        ctk.CTkLabel(zone_col, text="Zonas:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        self.selected_zones = [-1]
        self.btn_zone = ctk.CTkButton(zone_col, text="Global", width=80, command=self._open_zone_selector, fg_color="#334155", hover_color="#475569")
        self.btn_zone.pack(fill="x")

        # Operador Zona
        zop_col = ctk.CTkFrame(opt_frame, fg_color="transparent")
        zop_col.pack(side="left", fill="x", expand=True, padx=5)
        ctk.CTkLabel(zop_col, text="Lugar:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        self.sel_zop = ctk.CTkOptionMenu(zop_col, values=[z[0] for z in ZONE_OPERATORS], width=110)
        self.sel_zop.pack(fill="x")

        # Severidad
        sev_col = ctk.CTkFrame(opt_frame, fg_color="transparent")
        sev_col.pack(side="left", fill="x", expand=True, padx=5)
        ctk.CTkLabel(sev_col, text="Severidad:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        self.sel_severity = ctk.CTkOptionMenu(sev_col, values=[s[0] for s in SEVERITY_LEVELS], width=100)
        self.sel_severity.pack(fill="x")

        # Fila 4: Acciones (Checkboxes)
        act_frame = ctk.CTkFrame(frame, fg_color="transparent")
        act_frame.pack(fill="x", padx=12, pady=(8, 2))
        ctk.CTkLabel(act_frame, text="Acciones a disparar:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        
        checks_row = ctk.CTkFrame(act_frame, fg_color="transparent")
        checks_row.pack(fill="x")
        
        self.chk_log = ctk.CTkCheckBox(checks_row, text="Log", width=50)
        self.chk_log.pack(side="left", padx=(0, 5))
        self.chk_log.select()
        
        self.chk_telegram = ctk.CTkCheckBox(checks_row, text="Telegram", width=70)
        self.chk_telegram.pack(side="left", padx=5)
        
        self.chk_webhook = ctk.CTkCheckBox(checks_row, text="Webhook", width=70)
        self.chk_webhook.pack(side="left", padx=5)
        
        self.chk_tts = ctk.CTkCheckBox(checks_row, text="Voz", width=50)
        self.chk_tts.pack(side="left", padx=5)
        
        self.chk_photo = ctk.CTkCheckBox(checks_row, text="Foto", width=50)
        ...
        # Cooldown
        cd_col = ctk.CTkFrame(time_frame, fg_color="transparent")
        ...
        # Persistencia
        pers_col = ctk.CTkFrame(time_frame, fg_color="transparent")
        pers_col.pack(side="left", fill="x", expand=True, padx=(5, 0))
        ctk.CTkLabel(pers_col, text="Persistencia:", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(anchor="w")
        self.sel_persistence = ctk.CTkOptionMenu(pers_col, values=[p[0] for p in PERSISTENCE_PRESETS], width=90)
        self.sel_persistence.set("Instantaneo")
        self.sel_persistence.pack(fill="x")

        # Fila 4: Validacion Secundaria
        val_frame = ctk.CTkFrame(frame, fg_color="transparent")
        val_frame.pack(fill="x", padx=12, pady=(8, 2))

        # Proveedor
        prov_col = ctk.CTkFrame(val_frame, fg_color="transparent")
        prov_col.pack(side="left", fill="x", expand=True, padx=(0, 5))
        ctk.CTkLabel(prov_col, text="Validacion IA:", font=ctk.CTkFont(size=10), text_color="#a78bfa").pack(anchor="w")
        self.sel_validator = ctk.CTkOptionMenu(prov_col, values=[v[0] for v in VALIDATOR_PROVIDERS], 
                                                width=180, command=self._on_validator_change)
        self.sel_validator.pack(fill="x")

        # Prompt de validacion
        prompt_col = ctk.CTkFrame(val_frame, fg_color="transparent")
        prompt_col.pack(side="left", fill="x", expand=True, padx=(5, 0))
        ctk.CTkLabel(prompt_col, text="Prompt / Clase:", font=ctk.CTkFont(size=10), text_color="#a78bfa").pack(anchor="w")
        self.entry_val_prompt = ctk.CTkEntry(prompt_col, placeholder_text="Ej: persona, casco, fuego...")
    ...
    def _open_zone_selector(self):
        ...
        ctk.CTkLabel(top, text="Selecciona Zonas Objetivo", font=ctk.CTkFont(weight="bold")).pack(pady=10)
        ...
        chk_g = ctk.CTkCheckBox(scroll, text="Global", variable=var_g)
        chk_g.pack(anchor="w", pady=5)
        vars_dict[-1] = var_g
        
        for i in range(self.zones_count):
            var_i = ctk.BooleanVar(value=(i in self.selected_zones))
            chk_i = ctk.CTkCheckBox(scroll, text=f"Zona {i+1}", variable=var_i)
        ...
        ctk.CTkButton(top, text="Aceptar", command=apply).pack(pady=10)
    ...
    def _refresh_list(self):
        ...
        for rule in self.engine.rules:
            ...
            for s in SEVERITY_LEVELS:
                if s[0] == sev: color_sev = s[1]; break

            title_frame = ctk.CTkFrame(info, fg_color="transparent")
            title_frame.pack(fill="x")
            
            # Etiqueta severidad + nombre
            ctk.CTkLabel(title_frame, text=f" {sev.upper()} ", font=ctk.CTkFont(size=10, weight="bold"),
                         fg_color=color_sev, text_color="#0f172a", corner_radius=4).pack(side="left")
            ctk.CTkLabel(title_frame, text=f" {rule['name']}", font=ctk.CTkFont(size=12, weight="bold"),
                         text_color="#e2e8f0").pack(side="left", padx=(5, 0))
            
            # Acciones como tags visuales cortos
            ...
            if rule.get("save_evidence"):
                actions_txt += ",FOTO"
            ctk.CTkLabel(title_frame, text=f"[{actions_txt}]", font=ctk.CTkFont(size=10), text_color="#94a3b8").pack(side="left", padx=(5, 0))
            ...
            ctk.CTkLabel(info, text=detail, font=ctk.CTkFont(size=10), text_color="#64748b").pack(anchor="w")

            # Boton eliminar
            ctk.CTkButton(row, text="X", width=32, height=32,
    ...
    def _add_rule(self):
        try:
            ...
            # Resolver multiples acciones
            actions = []
            if self.chk_log.get(): actions.append("log")
            if self.chk_telegram.get(): actions.append("telegram")
            if self.chk_webhook.get(): actions.append("webhook")
            if self.chk_tts.get(): actions.append("tts")
