...
from ..utils.helpers import MODELS_DIR, ensure_dataset_structure, get_next_capture_filename, log_error, load_favorites, save_favorites
...
class AnnotationWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, frame=None, ds_name='', base_name='', dataset_dir='', on_save=None, on_close=None, image_files=None):
        ...
        self.sidebar = ctk.CTkFrame(self.main_container, width=180, fg_color='#1e293b', border_width=1, border_color='#334155', corner_radius=0)
        self.sidebar.pack(side='left', fill='y', padx=(0, 10))
        ctk.CTkLabel(self.sidebar, text='NUEVA CLASE', font=ctk.CTkFont(size=10, weight='bold'), text_color='#94a3b8', corner_radius=0).pack(pady=(10, 0))
        self.new_class_entry = ctk.CTkEntry(self.sidebar, height=28, font=ctk.CTkFont(size=11), placeholder_text='Nombre...', corner_radius=0)
        self.new_class_entry.pack(fill='x', padx=10, pady=(5, 0))
        self.new_class_entry.bind('<Return>', lambda e: self._add_new_class_from_sidebar())
        ctk.CTkButton(self.sidebar, text='AÑADIR CLASE', height=24, font=ctk.CTkFont(size=10, weight='bold'), fg_color='#334155', hover_color='#475569', command=self._add_new_class_from_sidebar, corner_radius=0).pack(pady=(0, 10), padx=10, fill='x')
        self.class_btn_frame = ctk.CTkScrollableFrame(self.sidebar, fg_color='transparent', corner_radius=0)
        self.class_btn_frame.pack(fill='both', expand=True, padx=5, pady=5)
        ctk.CTkLabel(self.sidebar, text='CONTEO TOTAL DATASET', font=ctk.CTkFont(size=10, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=(10, 0))
        self.summary_label = ctk.CTkLabel(self.sidebar, text='Calculando...', font=ctk.CTkFont(size=10), text_color='#94a3b8', justify='left', corner_radius=0)
        self.summary_label.pack(fill='x', padx=10, pady=(5, 10))
        self.canvas_container = ctk.CTkFrame(self.main_container, fg_color='black', corner_radius=0)
        ...
        self.info_label = ctk.CTkLabel(bottom, text='Cargando...', font=ctk.CTkFont(size=11), corner_radius=0)
        self.info_label.pack(side='left')
        if self.image_files:
            nav_frame = ctk.CTkFrame(bottom, fg_color='transparent', corner_radius=0)
            nav_frame.pack(side='right', padx=10)
            ctk.CTkButton(nav_frame, text='<', width=30, height=28, command=self._prev_image, fg_color='#334155', font=ctk.CTkFont(weight='bold'), corner_radius=0).pack(side='left', padx=0)
            self.index_label = ctk.CTkLabel(nav_frame, text=f' {self.current_index + 1} / {len(self.image_files)} ', font=ctk.CTkFont(size=11, weight='bold'), fg_color='#1e293b', corner_radius=0)
            self.index_label.pack(side='left', padx=0)
            ctk.CTkButton(nav_frame, text='>', width=30, height=28, command=self._next_image, fg_color='#334155', font=ctk.CTkFont(weight='bold'), corner_radius=0).pack(side='left', padx=0)
        save_btns = ctk.CTkFrame(bottom, fg_color='transparent', corner_radius=0)
        save_btns.pack(side='right', padx=0)
        ctk.CTkButton(save_btns, text='GUARDAR', width=100, height=30, command=self._save_and_stay, fg_color='#10b981', hover_color='#059669', font=ctk.CTkFont(size=11, weight='bold'), text_color='#0f172a', corner_radius=0).pack(side='left', padx=0)
        ctk.CTkButton(save_btns, text='GUARDAR Y SALIR', width=130, height=30, command=self._save_and_exit, fg_color='#3b82f6', hover_color='#2563eb', font=ctk.CTkFont(size=11, weight='bold'), text_color='#0f172a', corner_radius=0).pack(side='left', padx=0)
        if not self.image_files:
            ctk.CTkButton(save_btns, text='CANCELAR', width=90, height=30, command=self._cancel, fg_color='#450a0a', hover_color='#7f1d1d', font=ctk.CTkFont(size=11, weight='bold'), corner_radius=0).pack(side='left', padx=0)
        else:
            ctk.CTkButton(save_btns, text='FINALIZAR', width=110, height=30, command=self._finish, fg_color='#6366f1', hover_color='#4f46e5', font=ctk.CTkFont(size=11, weight='bold'), corner_radius=0).pack(side='left', padx=0)
        if self.image_files:
    ...
    def _refresh_class_list(self):
        ...
        for i, cls in enumerate(self.classes_found):
            ...
            btn = ctk.CTkButton(self.class_btn_frame, text=display_text, height=28, fg_color='#38bdf8' if is_active else '#334155', text_color='#0f172a' if is_active else '#e2e8f0', hover_color='#0ea5e9', command=lambda c=cls: self._set_active_class(c), corner_radius=0)
    ...
    def _on_release(self, event):
        ...
        if x2 - x1 < 8 or y2 - y1 < 8:
            if self.current_rect:
                self.ann_canvas.delete(self.current_rect)
            return
    ...
    def _update_yaml(self):
        ...
        content = [f'# Dataset: {self.ds_name}', ' # Generado por AI VisionSandbox Lab - Modo Multi-clase', f'path: {abs_path}', 'train: images/train', 'val: images/val', '', '# Clases', f'nc: {len(self.classes_found)}', 'names:']
...
class AddModelPopup(ctk.CTkToplevel):
    ...
    def __init__(self, parent, detector, on_success):
        ...
        self.grab_set()
        ctk.CTkLabel(self, text='GESTIÓN DE MODELOS', font=ctk.CTkFont(size=18, weight='bold'), corner_radius=0).pack(pady=(20, 15))
        self._label('1. Nombre de la Carpeta (Familia):')
        self.name_entry = ctk.CTkEntry(self, placeholder_text='Ej: YOLOv12, Trafico_Extremo', corner_radius=0)
        self.name_entry.pack(pady=(0, 15), padx=30, fill='x')
        self._label('2. Seleccionar archivo de pesos (.pt):')
        self.file_btn = ctk.CTkButton(self, text='Buscar archivo .pt', command=self._select_file, fg_color='#334155', corner_radius=0)
        self.file_btn.pack(pady=(0, 5), padx=30, fill='x')
        self.file_label = ctk.CTkLabel(self, text='Ningún archivo seleccionado', font=ctk.CTkFont(size=10), text_color='#888', corner_radius=0)
        self.file_label.pack(pady=(0, 15))
        self._label('3. Clases incrustadas en el modelo:')
        self.classes_label = ctk.CTkLabel(self, text='Selecciona un .pt para previsualizar sus clases', font=ctk.CTkFont(size=11), text_color='#aaa', corner_radius=0)
        self.classes_label.pack(pady=(0, 5), padx=30, anchor='w')
        self.classes_text = ctk.CTkTextbox(self, height=120, corner_radius=0)
        self.classes_text.pack(pady=(0, 20), padx=30, fill='x')
        self.classes_text.configure(state='disabled', fg_color='#1a1c1e')
        self.save_btn = ctk.CTkButton(self, text='Instalar Arquitectura', command=self._save_model, height=40, font=ctk.CTkFont(weight='bold'), fg_color='#16a34a', hover_color='#15803d', corner_radius=0)
        self.save_btn.pack(pady=20, padx=30, fill='x')
        self.save_btn.configure(state='disabled')

    def _label(self, text):
        ctk.CTkLabel(self, text=text, font=ctk.CTkFont(size=12, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=(5, 5), padx=30, anchor='w')
    ...
    def _save_model(self):
        ...
        if self.extracted_classes:
            ...
            meta = {'is_coco': is_coco, 'classes': None if is_coco else self.extracted_classes}
        else:
...
class ClassFilterWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, detector, current_targets, on_apply):
        ...
        if current_targets is not None:
            for tid in current_targets:
                try:
                    self.selected_ids.add(int(tid))
                except:
                    pass
        else:
            self.selected_ids = set(self.all_classes.keys())
        ctk.CTkLabel(self, text='SELECCIÓN DE CLASES', font=ctk.CTkFont(size=16, weight='bold'), corner_radius=0).pack(pady=(15, 5))
        ctk.CTkLabel(self, text='Desmarca las clases que NO quieres detectar', text_color='#aaa', font=('', 11), corner_radius=0).pack(pady=(0, 10))
        btn_frame = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        btn_frame.pack(fill='x', padx=20, pady=5)
        ctk.CTkButton(btn_frame, text='Marcar Todas', command=self._check_all, width=120, corner_radius=0).pack(side='left', padx=5)
        ctk.CTkButton(btn_frame, text='Desmarcar Todas', command=self._uncheck_all, width=120, fg_color='#4b5563', corner_radius=0).pack(side='right', padx=5)
        self.scroll = ctk.CTkScrollableFrame(self, corner_radius=0)
        self.scroll.pack(fill='both', expand=True, padx=20, pady=10)
        self.search_var = ctk.StringVar()
        self.search_var.trace('w', self._on_search)
        self.search_entry = ctk.CTkEntry(self, placeholder_text='Buscar clase...', textvariable=self.search_var, corner_radius=0)
        self.search_entry.pack(fill='x', padx=20, pady=(0, 10))
        self._build_list()
        ctk.CTkButton(self, text='Aplicar Filtro', command=self._apply, fg_color='#16a34a', hover_color='#15803d', font=('', 14, 'bold'), height=40, corner_radius=0).pack(fill='x', padx=20, pady=(0, 15))
    ...
    def _build_list(self, filter_text=''):
        ...
        if not self.all_classes:
            lbl = ctk.CTkLabel(self.scroll, text='No se encontraron clases.', text_color='#666', corner_radius=0)
            lbl.pack(pady=20)
        ...
        for class_id, class_name in sorted(self.all_classes.items(), key=lambda x: x[1]):
            ...
            cb = ctk.CTkCheckBox(self.scroll, text=f'{class_name.capitalize()} (ID: {class_id})', variable=var, corner_radius=0)
...
class InfoWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        self.resizable(False, False)
        self.grab_set()
        ctk.CTkLabel(self, text='AI VISIONSANDBOX LAB', font=ctk.CTkFont(size=18, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=(20, 2))
        ctk.CTkLabel(self, text='v4.0 - Motor de Vision Artificial en Tiempo Real', font=ctk.CTkFont(size=11), text_color='#94a3b8', corner_radius=0).pack(pady=(0, 2))
        ...
        self.show_dep_btn = ctk.CTkButton(self.dep_frame, text='Ver Sub-librerias', command=self._toggle_deps, fg_color='#334155', corner_radius=0)
        self.show_dep_btn.pack(pady=5)
        self.dep_textbox = ctk.CTkTextbox(self.dep_frame, height=150, fg_color='#1a1c1e', corner_radius=0)
        ...
        def get_v(pkg):
            try:
                ...
                return 'N/A'
        deps = [f"PyTorch (v{get_v('torch')})", f"NumPy (v{get_v('numpy')})", f"yt-dlp (v{get_v('yt-dlp')})", f"requests (v{get_v('requests')})", f"LapX - Tracking ByteTrack (v{get_v('lapx')})", f"OpenVINO - Aceleracion Intel (v{get_v('openvino')})", f"scipy (v{get_v('scipy')})", f'SQLite3 (incluido en Python)']
        self.dep_textbox.insert('0.0', '\n'.join(deps))
        self.dep_textbox.configure(state='disabled')
        self._section('Funcionalidades Clave')
        features = 'Deteccion YOLO/RT-DETR + Zero-Shot (YOLO-World)\nTracking de objetos con ByteTrack + Focus Mode\nZonas poligonales con motor de hitos y alertas\nTelegram, Webhooks, TTS y evidencias automaticas\nDashboard interactivo con graficas por zonas\nAnotador visual con atajos de teclado\nAutocaptura, importacion ZIP y exportacion de datasets\nAceleracion GPU: CUDA / OpenVINO / DirectML'
        ctk.CTkLabel(self, text=features, font=ctk.CTkFont(size=10), text_color='#94a3b8', justify='left', corner_radius=0).pack(padx=40, anchor='w')
        ctk.CTkButton(self, text='Cerrar', command=self.destroy, fg_color='#6b7280', hover_color='#4b5563', corner_radius=0).pack(pady=(15, 20))

    def _section(self, title):
        ctk.CTkLabel(self, text=title, font=ctk.CTkFont(weight='bold', size=13), corner_radius=0).pack(padx=30, pady=(12, 3), anchor='w')
...
class CaptureNamePopup(ctk.CTkToplevel):
    ...
    def __init__(self, parent, on_accept, on_cancel=None):
        ...
        self.resizable(False, False)
        self.grab_set()
        ctk.CTkLabel(self, text='NOMBRE DEL OBJETO / ETIQUETA', font=ctk.CTkFont(size=12, weight='bold'), corner_radius=0).pack(pady=(20, 10))
        self.entry = ctk.CTkEntry(self, placeholder_text='ej: taxi, persona, bache', width=250, corner_radius=0)
        self.entry.pack(pady=10)
        self.entry.bind('<Return>', lambda _: self._accept())
        self.entry.focus()
        btn_row = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        btn_row.pack(pady=10)
        ctk.CTkButton(btn_row, text='Continuar', width=100, command=self._accept, fg_color='#16a34a', hover_color='#15803d', corner_radius=0).pack(side='left', padx=5)
        ctk.CTkButton(btn_row, text='Cancelar', width=80, command=self._cancel, fg_color='#6b7280', hover_color='#4b5563', corner_radius=0).pack(side='left', padx=5)
...
class ModelExplorerWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, detector):
        ...
        if 'openvino' in self.detector.hardware_diag['best_backend']:
            accel_frame = ctk.CTkFrame(self, fg_color='#1a1c1e', border_width=1, border_color='#38bdf8', corner_radius=0)
            accel_frame.pack(fill='x', padx=20, pady=(0, 10))
            ctk.CTkLabel(accel_frame, text='ACELERACION INTEL DETECTADA', font=ctk.CTkFont(size=11, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=(10, 5))
            self.btn_opt = ctk.CTkButton(accel_frame, text='Acelerar Modelo Actual (OpenVINO)', command=self._optimize_active, fg_color='#0369a1', hover_color='#075985', corner_radius=0)
    ...
    def _refresh(self):
        ...
        if not families:
            ctk.CTkLabel(self.scroll, text='No hay modelos instalados.', text_color='#666', corner_radius=0).pack(pady=40)
            return
        for fam in sorted(families):
            f_path = os.path.join(MODELS_DIR, fam)
            models = [f for f in os.listdir(f_path) if f.endswith('.pt')]
            card = ctk.CTkFrame(self.scroll, fg_color='#1e293b', border_width=1, border_color='#334155', corner_radius=0)
            card.pack(fill='x', pady=5, padx=2)
            lbl_title = ctk.CTkLabel(card, text=fam.upper(), font=ctk.CTkFont(weight='bold'), text_color='#38bdf8', corner_radius=0)
            lbl_title.pack(side='left', padx=15, pady=12)
            lbl_count = ctk.CTkLabel(card, text=f'{len(models)} pesos (.pt)', font=('', 10), text_color='#94a3b8', corner_radius=0)
            lbl_count.pack(side='left', padx=5)
            ctk.CTkButton(card, text='X', width=30, height=30, fg_color='#7f1d1d', hover_color='#991b1b', command=lambda f=fam: self._delete_family(f), corner_radius=0).pack(side='right', padx=10)
            ctk.CTkButton(card, text='+ Pesos', width=80, height=30, fg_color='#334155', hover_color='#475569', command=lambda f=fam: self._add_weights(f), corner_radius=0).pack(side='right', padx=5)
...
class SourceSelectorWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, current_url, on_change):
        ...
        self.resizable(False, False)
        self.grab_set()
        ctk.CTkLabel(self, text='CONFIGURAR FUENTE DE ENTRADA', font=ctk.CTkFont(size=18, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=(25, 20))
        self._section_title('STREAM EN DIRECTO (RTSP, RTMP, HLS)')
        live_frame = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        live_frame.pack(fill='x', padx=40, pady=(0, 15))
        self.live_entry = ctk.CTkEntry(live_frame, placeholder_text='rtsp://... o URL de directo', height=32, corner_radius=0)
        self.live_entry.pack(side='left', fill='x', expand=True, padx=(0, 8))
        ctk.CTkButton(live_frame, text='Conectar', width=90, height=32, fg_color='#1e293b', hover_color='#334155', text_color='#38bdf8', command=lambda: self._apply(self.live_entry.get()), corner_radius=0).pack(side='right')
        self._section_title('VIDEO REDIFERIDO (YouTube, Vimeo, MP4 URL)')
        vod_frame = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        vod_frame.pack(fill='x', padx=40, pady=(0, 15))
        self.vod_entry = ctk.CTkEntry(vod_frame, placeholder_text='https://www.youtube.com/watch?v=...', height=32, corner_radius=0)
        self.vod_entry.pack(side='left', fill='x', expand=True, padx=(0, 8))
        ctk.CTkButton(vod_frame, text='Cargar', width=90, height=32, fg_color='#1e293b', hover_color='#334155', text_color='#38bdf8', command=lambda: self._apply(self.vod_entry.get()), corner_radius=0).pack(side='right')
        self._section_title('ARCHIVO LOCAL')
        ctk.CTkButton(self, text='Seleccionar Archivo de Vídeo', height=35, fg_color='#334155', hover_color='#475569', command=self._browse_local, corner_radius=0).pack(fill='x', padx=40, pady=(0, 15))
        self._section_title('CAMARAS LOCALES (USB / INTEGRADA)')
        cam_frame = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        cam_frame.pack(fill='x', padx=40, pady=(0, 20))
        self.cam_combo = ctk.CTkComboBox(cam_frame, values=['Presiona Escanear...'], state='readonly', height=32, corner_radius=0)
        self.cam_combo.pack(side='left', fill='x', expand=True, padx=(0, 8))
        self.cam_combo.set('Escanear camaras...')
        btn_scan = ctk.CTkButton(cam_frame, text='Escanear', width=70, height=32, fg_color='#334155', hover_color='#475569', command=self._scan_cameras, corner_radius=0)
        btn_scan.pack(side='left', padx=(0, 8))
        ctk.CTkButton(cam_frame, text='Cargar', width=70, height=32, fg_color='#1e293b', hover_color='#334155', text_color='#38bdf8', command=self._apply_camera, corner_radius=0).pack(side='right')
        ...
        ctk.CTkButton(self, text='Cerrar', command=self.destroy, fg_color='#6b7280', hover_color='#4b5563', corner_radius=0).pack(pady=10)

    def _section_title(self, text):
        ctk.CTkLabel(self, text=text, font=ctk.CTkFont(size=12, weight='bold'), text_color='#94a3b8', corner_radius=0).pack(padx=40, pady=(5, 5), anchor='w')
...
class FavoritesWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, on_select):
        ...
        self.grab_set()
        ctk.CTkLabel(self, text='FUENTES FAVORITAS', font=ctk.CTkFont(size=16, weight='bold'), text_color='#38bdf8', corner_radius=0).pack(pady=20)
        self.scroll = ctk.CTkScrollableFrame(self, fg_color='transparent', corner_radius=0)
        self.scroll.pack(fill='both', expand=True, padx=20, pady=10)
        self._refresh()
    ...
    def _refresh(self):
        ...
        favs = load_favorites()
        if not favs:
            ctk.CTkLabel(self.scroll, text='No tienes fuentes guardadas.', text_color='#666', corner_radius=0).pack(pady=40)
            return
        for i, f in enumerate(favs):
            card = ctk.CTkFrame(self.scroll, fg_color='#1e293b', border_width=1, border_color='#334155', corner_radius=0)
            card.pack(fill='x', pady=6, padx=5)
            text_frame = ctk.CTkFrame(card, fg_color='transparent', corner_radius=0)
            text_frame.pack(side='left', fill='both', expand=True, padx=15, pady=10)
            name_lbl = ctk.CTkLabel(text_frame, text=f['name'].upper(), font=ctk.CTkFont(size=13, weight='bold'), text_color='#38bdf8', anchor='w', corner_radius=0)
            name_lbl.pack(fill='x')
            url_display = f['url']
            if len(url_display) > 55:
                url_display = url_display[:52] + '...'
            url_lbl = ctk.CTkLabel(text_frame, text=url_display, font=('', 10), text_color='#94a3b8', anchor='w', corner_radius=0)
            url_lbl.pack(fill='x')
            btn_frame = ctk.CTkFrame(card, fg_color='transparent', corner_radius=0)
            btn_frame.pack(side='right', padx=10)
            ctk.CTkButton(btn_frame, text='Cargar', width=75, height=30, fg_color='#38bdf8', hover_color='#0ea5e9', text_color='#000', font=ctk.CTkFont(size=12, weight='bold'), command=lambda u=f['url']: self._apply(u), corner_radius=0).pack(side='left', padx=5)
            ctk.CTkButton(btn_frame, text='\ue74d', width=35, height=30, fg_color='#450a0a', hover_color='#7f1d1d', font=ctk.CTkFont(family='Segoe MDL2 Assets', size=14), command=lambda idx=i: self._delete(idx), corner_radius=0).pack(side='left')
...
class CaptureNamePopup(ctk.CTkToplevel):
    ...
    def __init__(self, parent, on_accept, on_cancel=None, title='Etiquetar Objeto', prompt='ETIQUETA DEL OBJETO'):
        ...
        self.geometry(f'+{x}+{y}')
        ctk.CTkLabel(self, text=prompt, font=ctk.CTkFont(size=12, weight='bold'), text_color='#38bdf8', justify='center', corner_radius=0).pack(pady=(20, 10))
        self.entry = ctk.CTkEntry(self, placeholder_text='ej: pez, burbuja, coral...', width=280, corner_radius=0)
        self.entry.pack(pady=10)
        self.entry.focus_set()
        self.entry.bind('<Return>', lambda e: self._accept())
        btn_row = ctk.CTkFrame(self, fg_color='transparent', corner_radius=0)
        btn_row.pack(pady=15)
        ctk.CTkButton(btn_row, text='Aceptar', width=100, command=self._accept, fg_color='#16a34a', hover_color='#15803d', corner_radius=0).pack(side='left', padx=5)
        ctk.CTkButton(btn_row, text='Cancelar', width=100, command=self._cancel, fg_color='#334155', corner_radius=0).pack(side='left', padx=5)
        self.protocol('WM_DELETE_WINDOW', self._cancel)
